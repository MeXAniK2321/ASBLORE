...
function OnAnimeShuffleSended()
{
	OnAutoAssignPressed();
	OnShufflePlayersPressed();
	//OnLockAndStartPressed();

	//$.Schedule(1, OnAnimeShuffleSended);
}
//--------------------------------------------------------------------------------------------------
...
(function()
{
	...
	GameEvents.Subscribe( "anime_shuffle_teams", OnAnimeShuffleSended );

	if (Game.GetMapInfo().map_display_name == "anime_5x15_shuffle")
	{
		$.Schedule(1, OnAnimeShuffleSended);
		$.Schedule(2, OnAnimeShuffleSended);
		$.Schedule(3, OnAnimeShuffleSended);
		$.Schedule(4, OnAnimeShuffleSended);
	}
	//$.Msg(Game.GetMapInfo().map_display_name, "MAP")
})();
