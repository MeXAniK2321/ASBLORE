...
nanaya_clones1:ComboD(caster, target)
end
end
...
function nanaya_clones1:ComboD(caster, target)
...
ParticleManager:SetParticleControl(knife, 4, target:GetAbsOrigin())
	...
	nanaya_clones1:clones_2D(caster, target, knockback_push1)
	end
	end)
	end)
	--print (secondtime)
	...
	function nanaya_clones1:clones_2D(caster, target, knockback_push1)
	local table = {12, 13, 12}
	local numberhit = 0
	local dmg = 150 + math.floor(caster:GetAgility()*2)
	Timers:CreateTimer(0.1, function()
											if numberhit < 3 then
