function invoker_chaos_meteor_datadriven_on_spell_start(keys)
	...
	keys.caster:EmitSound("tengai.Shinsei")
	...
	local meteor_fly_original_point = (target_point - (velocity_per_second * keys.LandTime)) + Vector (500, 0, 2000)  --Start the meteor in the air in a place where it'll be moving the same speed when flying and when rolling.
	local chaos_meteor_fly_particle_effect = ParticleManager:CreateParticle("particles/tengai_shinsei_fly.vpcf", PATTACH_ABSORIGIN, keys.caster)
	...
	Timers:CreateTimer({
		...
		callback = function()
			...
			chaos_meteor_dummy_unit:EmitSound("")  --Emit a sound that will follow the meteor.
			...
			{
				EffectName = "particles/tengai_shinsei.vpcf",
