item_seal_of_chaos = item_seal_of_chaos or class({})
...
---------------------------------------------------------------------------------------------------------------------
modifier_item_seal_of_chaos = modifier_item_seal_of_chaos or class({})
...
modifier_item_seal_of_chaos_buff_invul = modifier_item_seal_of_chaos_buff_invul or class({})
...
modifier_item_seal_of_chaos_buff_astral = modifier_item_seal_of_chaos_buff_astral or class({})

--------------------------------------------------------------------------------
-- Classifications
function modifier_item_seal_of_chaos_buff_astral:IsHidden() return false end
function modifier_item_seal_of_chaos_buff_astral:IsDebuff() return false end
function modifier_item_seal_of_chaos_buff_astral:IsStunDebuff() return true end
function modifier_item_seal_of_chaos_buff_astral:IsPurgable() return true end
function modifier_item_seal_of_chaos_buff_astral:RemoveOnDeath() return false end
--------------------------------------------------------------------------------
-- Initializations
function modifier_item_seal_of_chaos_buff_astral:OnCreated( kv )
    self.parent = self:GetParent()
	
	self:PlayEffects(true, true)
...
function modifier_item_seal_of_chaos_buff_astral:OnDestroy()
	...
	self:PlayEffects(false, false)
	self.parent:Kill(self,caster)
end
...
--------------------------------------------------------------------------------
-- Graphics & Animations
function modifier_item_seal_of_chaos_buff_astral:PlayEffects(bParticle, bSound)
	-- Get Resources
	local particle_cast1 = bParticle 
	                       and "particles/fatality_murder.vpcf"
						   or "particles/fatality.vpcf"
    
	if bSound then  
	  EmitSoundOn("fatality.2_1", self.parent)
      EmitSoundOn("fatality.2_2", self.parent)
    else
      EmitSoundOn("fatality.3", self.parent)
      EmitSoundOn("fatality.3_1", self.parent)
	end
	-- Create Particle
	local effect_cast1 = ParticleManager:CreateParticle( particle_cast1, PATTACH_WORLDORIGIN, nil )
