    ...
    if IsServer() then
        if me:IsMoving()
			...
			if not self.parent:IsDisarmed()
				...
				local fAttacksPerSecond = self.bIsDrifting and ( math.ceil(self.parent:GetAttacksPerSecond(false)) ) or 1
				...
				for _, hEnemy in pairs(hEnemies) do
					if IsNotNull(hEnemy) then
						...
						if not self.hDriftEnemies[iEntIndex] then
							self.hDriftEnemies[iEntIndex] = self.bIsDrifting and 1 or 1.5
							...
							for i = 1, math.min(fAttacksPerSecond, 3) do
							...
							if self.bIsDrifting then
								EmitSoundOn("AE86.Drift.Impact.1", hEnemy)
							else
							    EmitSoundOn("jetstream.crit"..RandomInt(1, 2), hEnemy)
							end
						end
					end
				end
...
function modifier_ae86_repair:OnCreated(hTable)
    ...
    if IsServer() then
        if not self.RepairFX then
		    self.parent:StartGestureWithPlaybackRate(ACT_DOTA_CAST_ABILITY_1, 1.0)
...
function modifier_ae86_theme:OnCreated(hTable)
    ...
    if IsServer() then
	    if IsASBPatreon(self.parent) then
		    EmitSoundOn("AE86.Theme.Arcana.Thunder", self.parent)
		    EmitSoundOn("AE86.Theme.Arcana."..RandomInt(1, 5), self.parent)
			if not self.ThemeFX1 then
				local vColor =  Vector(255, 255, 255)

				self.ThemeFX1 = ParticleManager:CreateParticle( "particles/heroes/anime_hero_ae86/arcana/alphalucia_theme.vpcf", PATTACH_ABSORIGIN_FOLLOW, self.parent )
								...
								ParticleManager:SetParticleControl(self.ThemeFX1, 11, Vector(1, 0, 0))
...
function modifier_ae86_theme:OnDestroy()
    if IsServer() then
        ...
        for i = 1, 5 do
            StopSoundOn("AE86.Theme.Arcana."..i, self.parent)
        end
    end
end
