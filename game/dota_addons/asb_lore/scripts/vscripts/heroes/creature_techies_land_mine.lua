...
LinkLuaModifier( "modifier_creature_techies_land_mine", "heroes/modifier_creature_techies_land_mine", LUA_MODIFIER_MOTION_NONE )

--------------------------------------------------------------------------------

function creature_techies_land_mine:OnSpellStart()
	if IsServer() then
		local hMine = CreateUnitByName( "npc_dota_bench", self:GetCursorPosition(), true, self:GetCaster(), self:GetCaster(), self:GetCaster():GetTeamNumber() )
		if hMine ~= nil then
