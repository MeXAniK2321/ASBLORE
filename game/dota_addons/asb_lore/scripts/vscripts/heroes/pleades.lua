...
LinkLuaModifier( "modifier_pleades_thinker", "heroes/pleades.lua", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_air_banner", "heroes/pleades.lua", LUA_MODIFIER_MOTION_NONE )

--------------------------------------------------------------------------------

function pleades:Spawn()
    if IsServer() then
        self:SetThink( "OnIntervalThink", self, 0.25 )
    end
end
function pleades:GetIntrinsicModifierName()
    return "modifier_air_banner"
end
function pleades:OnUpgrade()
...
function pleades:OnIntervalThink()
    if IsServer() then
...
function pleades:IsStealable()
   return false
end

function pleades:GetAOERadius()
    return self:GetSpecialValueFor("radius")
end

function pleades:GetBehavior()
    return DOTA_ABILITY_BEHAVIOR_POINT + DOTA_ABILITY_BEHAVIOR_DONT_RESUME_ATTACK + DOTA_ABILITY_BEHAVIOR_AOE
end

function pleades:OnSpellStart()
    if IsServer() then
        local target = CreateModifierThinker(self:GetCaster(), self, "modifier_pleades_thinker", nil, self:GetCursorPosition(), self:GetCaster():GetTeamNumber(), false)

        local info = {
			EffectName = "particles/pleades_proj_true.vpcf",
			Ability = self,
        ...
        EmitSoundOn( "jellal.4_2", self:GetCaster() )
    end
	if IsServer() then
...
function pleades:OnProjectileHit( hTarget, vLocation )
    local caster = self:GetCaster()
	if hTarget ~= nil then
        EmitSoundOn( "magic.explosion", hTarget )

        if IsServer() then
            local caster = self:GetCaster()
  
          local particle_fx = "particles/pleades_explosion.vpcf"
...
modifier_pleades_thinker = class ({})

function modifier_pleades_thinker:CheckState()
