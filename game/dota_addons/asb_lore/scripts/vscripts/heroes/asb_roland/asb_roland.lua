...
--!!----------------------------------------------------------------------------------------------------------------------------------------------------------
local function R_StringSplit(sString, sSplitter)
    local t = {}
...
---------------------------------------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------------------------------
roland_axe_bonk = roland_axe_bonk or class({})

function roland_axe_bonk:IsStealable()                              return true end
function roland_axe_bonk:IsHiddenWhenStolen()                       return false end
function roland_axe_bonk:GetCastPoint()
    local hCaster = self:GetCaster()
    return hCaster:HasModifier("modifier_roland_recast_time") and 0 or self.BaseClass.GetCastPoint(self)
end
function roland_axe_bonk:GetBehavior()
    local hCaster = self:GetCaster()
    return hCaster:HasModifier("modifier_roland_recast_time") and self:R_GetRecastBehavior() or self.BaseClass.GetBehavior(self)
end
function roland_axe_bonk:GetCastRange(vLocation, hTarget)
    return self:GetSpecialValueFor("cast_range")
end
function roland_axe_bonk:GetAOERadius()
    return self:GetSpecialValueFor("aoe_optional")
end
function roland_axe_bonk:OnUpgrade()
    if IsServer() then
        local hCaster = self:GetCaster()
        if not IsNotNull(hCaster) then return end
        local hModifierLightControll = hCaster:FindModifierByNameAndCaster("modifier_roland_light_controller", hCaster)
        if not IsNotNull(hModifierLightControll) then return end
        if type(hModifierLightControll.R_OnUpgrade) == "function" then
            hModifierLightControll:R_OnUpgrade(self)
        end
    end
end
function roland_axe_bonk:OnAbilityPhaseStart()
    local hCaster = self:GetCaster()
    hCaster:ClearActivityModifiers()
    hCaster:AddActivityModifier(self:GetAbilityName())
    return true
end
function roland_axe_bonk:_R_IsOwnerStaminaEnough()
    local hCaster = self:GetCaster()
    return hCaster:GetModifierStackCount("modifier_roland_light_controller", hCaster) >= self:R_GetStaminaCost()
end
function roland_axe_bonk:CastFilterResult()
    if not self:_R_IsOwnerStaminaEnough() then
        return UF_FAIL_CUSTOM
    end
    return self.BaseClass.CastFilterResult(self)
end
function roland_axe_bonk:CastFilterResultTarget(hTarget)
    if not self:_R_IsOwnerStaminaEnough() then
        return UF_FAIL_CUSTOM
    end
    return self.BaseClass.CastFilterResultTarget(self, hTarget)
end
function roland_axe_bonk:CastFilterResultLocation(vLocation)
    if not self:_R_IsOwnerStaminaEnough() then
        return UF_FAIL_CUSTOM
    end
    return self.BaseClass.CastFilterResultLocation(self, vLocation)
end
-- function roland_axe_bonk:GetAnimeVectorTargetingRange()
--     return 300--self:GetSpecialValueFor("distance")
-- end
-- function roland_axe_bonk:GetAnimeVectorTargetingStartRadius()
--     return 100--self:GetSpecialValueFor("width")
-- end
-- function roland_axe_bonk:GetAnimeVectorTargetingEndRadius()
--     return 300--self:GetSpecialValueFor("width")
-- end
-- function roland_axe_bonk:GetAnimeVectorTargetingColor()
--     return Vector(0, 255, 0)
-- end
-- function roland_axe_bonk:IsAnimeVectorTargetingIgnoreWidth()
--     return true
-- end
-- function roland_axe_bonk:IsAnimeVectorTargetingDual()
--     return true
-- end
function roland_axe_bonk:OnSpellStart()
    local hCaster             = self:GetCaster()
    local iCasterTeam         = hCaster:GetTeamNumber()
    --local vMainPosition       = self:GetCursorPosition()
    --local vTargetingDirection = self:GetAnimeVectorTargetingMainDirection()

    local nRecastTime = self:R_GetRecastTime()

    if hCaster:HasModifier("modifier_roland_recast_time") then
        hCaster:RemoveModifierByNameAndCaster("modifier_roland_recast_time", hCaster)

        self:R_OnCast(hCaster, true)
    else
        if self:R_GetDashDuration() > 0 and self:R_GetDashDistance() > 0 then
            hCaster:AddNewModifier(hCaster, self, "modifier_roland_dash_motion",    {
                                                                                        duration      = self:R_GetDashDuration(),
                                                                                        nDashDistance = self:R_GetDashDistance(),
                                                                                        nRecastTime   = nRecastTime,
                                                                                        nIsReturn     = 0,

                                                                                        vPoint = self:R_GetDashPoint()
                                                                                    })
        else
            if nRecastTime > 0 then
                hCaster:AddNewModifier(hCaster, self, "modifier_roland_recast_time", {duration = nRecastTime})
            else
                local hModifierLightControll = hCaster:FindModifierByNameAndCaster("modifier_roland_light_controller", hCaster)
                if not IsNotNull(hModifierLightControll) then return end
                if type(hModifierLightControll.R_RandomNextAbility) == "function" then
                    hModifierLightControll:R_RandomNextAbility(self)
                end
            end  
        end

        self:R_OnCast(hCaster, false)
    end
end
--=========================================================-- This need use for your logic, all of them basicaly can be overriden from children classes
function roland_axe_bonk:R_GetRecastBehavior()
    return self.BaseClass.GetBehavior(self)
end
function roland_axe_bonk:R_GetRecastTime()
    return self:GetSpecialValueFor("r_recast_time")
end
function roland_axe_bonk:R_CanBeRandomed()
    return true
end
function roland_axe_bonk:R_GetStaminaCost()
    return self:GetSpecialValueFor("r_stamina_cost")
end
function roland_axe_bonk:R_GetDashDuration()
    return self:GetSpecialValueFor("r_dash_duration")
end
function roland_axe_bonk:R_GetDashOffset()
    return self:GetSpecialValueFor("r_dash_offset")
end
function roland_axe_bonk:R_GetDashDistance()
    return math.max(0, self:GetCastRange(self:GetCaster():GetAbsOrigin(), self:GetCaster()) - self:R_GetDashOffset())
end
function roland_axe_bonk:R_GetDashPoint()
    return self:GetCursorPosition()
end
--=========================================================-- This need use for your logic
function roland_axe_bonk:R_OnWhileDash(hModifier, vLocation)

end
function roland_axe_bonk:R_OnAfterDash(hModifier, vLocation)

end
function roland_axe_bonk:R_OnCast(hCaster, bRecast)
...
---------------------------------------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------------------------------
roland_hammer_stun = roland_hammer_stun or class(roland_axe_bonk)

---------------------------------------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------------------------------
roland_sword_doubleswing = roland_sword_doubleswing or class(roland_axe_bonk)

---------------------------------------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------------------------------
roland_katana_swing = roland_katana_swing or class(roland_axe_bonk)

---------------------------------------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------------------------------
roland_jc = roland_jc or class(roland_axe_bonk)

---------------------------------------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------------------------------
roland_cross_strike = roland_cross_strike or class(roland_axe_bonk)

---------------------------------------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------------------------------
roland_gun_doubleshot = roland_gun_doubleshot or class(roland_axe_bonk)

---------------------------------------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------------------------------
roland_spear_knock = roland_spear_knock or class(roland_axe_bonk)

---------------------------------------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------------------------------
roland_big_sword_stun = roland_big_sword_stun or class(roland_axe_bonk)
...
---------------------------------------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------------------------------
roland_pandora_box = roland_pandora_box or class({})

function roland_pandora_box:IsStealable()                              return true end
function roland_pandora_box:IsHiddenWhenStolen()                       return false end
function roland_pandora_box:OnSpellStart()
...
---------------------------------------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------------------------------
roland_ultimate = roland_ultimate or class({})

function roland_ultimate:IsStealable()                              return true end
function roland_ultimate:IsHiddenWhenStolen()                       return false end
function roland_ultimate:Spawn()
    if IsServer() then
        local hCaster  = self:GetCaster()
        local hAbility = self
        Timers:CreateTimer(0, function()
            if IsNotNull(hCaster) and IsNotNull(hAbility) then
                hAbility:SetLevel(hAbility:GetMaxLevel())
            end
        end)
    end
end
function roland_ultimate:OnSpellStart()
    local hCaster = self:GetCaster()

    hCaster:RemoveModifierByName("modifier_roland_ultimate")
end
---------------------------------------------------------------------------------------------------------------------
LinkLuaModifier("modifier_roland_ultimate", "heroes/asb_roland/asb_roland", LUA_MODIFIER_MOTION_NONE)

modifier_roland_ultimate = modifier_roland_ultimate or class({})

function modifier_roland_ultimate:IsHidden()                                                                    return true end
function modifier_roland_ultimate:IsDebuff()                                                                    return false end
function modifier_roland_ultimate:IsPurgable()                                                                  return false end
function modifier_roland_ultimate:IsPurgeException()                                                            return false end
function modifier_roland_ultimate:RemoveOnDeath()                                                               return true end
function modifier_roland_ultimate:DeclareFunctions()
    local tFunc =   {
                    }
    return tFunc
end
function modifier_roland_ultimate:OnCreated(tTable)
    self.hCaster  = self:GetCaster()
    self.hParent  = self:GetParent()
    self.hAbility = self:GetAbility()

    if IsServer() then
        self.hParent:SwapAbilities("roland_pandora_box", "roland_ultimate", false, true)
    end
end
function modifier_roland_ultimate:OnRefresh(tTable)
    self:OnCreated(tTable)
end
function modifier_roland_ultimate:OnDestroy()
    if IsServer() and IsNotNull(self.hParent) then
        local hPandoraBox = self.hParent:FindAbilityByName("roland_pandora_box")
        local bIsUnHidden = IsNotNull(hPandoraBox) and hPandoraBox:IsTrained()

        self.hParent:SwapAbilities("roland_pandora_box", "roland_ultimate", bIsUnHidden, false)
    end
end
...
---------------------------------------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------------------------------
roland_light_controller = roland_light_controller or class({})

function roland_light_controller:IsStealable()                              return true end
function roland_light_controller:IsHiddenWhenStolen()                       return false end
function roland_light_controller:Spawn()
    if IsServer() then
        local hCaster  = self:GetCaster()
        local hAbility = self
        Timers:CreateTimer(0, function()
...
function roland_light_controller:GetIntrinsicModifierName()
    return "modifier_roland_light_controller"
end
function roland_light_controller:OnSpellStart()

end
---------------------------------------------------------------------------------------------------------------------
LinkLuaModifier("modifier_roland_light_controller", "heroes/asb_roland/asb_roland", LUA_MODIFIER_MOTION_NONE)

modifier_roland_light_controller = modifier_roland_light_controller or class({})

function modifier_roland_light_controller:IsHidden()                                                                    return false end
function modifier_roland_light_controller:IsDebuff()                                                                    return false end
function modifier_roland_light_controller:IsPurgable()                                                                  return false end
function modifier_roland_light_controller:IsPurgeException()                                                            return false end
function modifier_roland_light_controller:RemoveOnDeath()                                                               return false end
function modifier_roland_light_controller:DestroyOnExpire()                                                             return false end
function modifier_roland_light_controller:DeclareFunctions()
    local tFunc =   {
...
function modifier_roland_light_controller:OnAbilityExecuted(keys)
    if keys.unit == self.hParent then
        if IsNotNull(keys.ability) and type(keys.ability.R_GetStaminaCost) == "function" and not keys.unit:HasModifier("modifier_roland_recast_time") then 
            local nStaminaCost = keys.ability:R_GetStaminaCost()
            if nStaminaCost > 0 then
                local nNewStacks = math.max(0, self:GetStackCount() - nStaminaCost)

                self:SetStackCount(nNewStacks)

                if self:GetDuration() <= -1 then
                    self:StartIntervalThink(self.nStaminaRecoverTime)
                    self:SetDuration(self.nStaminaRecoverTime, true)
                end
            end

            local hModifierCasts = keys.unit:FindModifierByNameAndCaster("modifier_roland_light_max_casts_controll", self.hCaster)
            if IsNotNull(hModifierCasts) then
                hModifierCasts:UseCast()
            else
                hModifierCasts = keys.unit:AddNewModifier(self.hCaster, self.hAbility, "modifier_roland_light_max_casts_controll", {duration = self.nTimeForCasts, nMaxCasts = self.nMaxCasts, vStartPoint = self.hParent:GetAbsOrigin(), nReturnTime = self.nReturnTime})
            end
        end
    end
end
function modifier_roland_light_controller:OnDeath(keys)
    if keys.unit == self.hParent then
        self._tFURIOSO_ULTIMATE_UNLOCK = {}
    end
end
function modifier_roland_light_controller:OnCreated(tTable)
    self.hCaster  = self:GetCaster()
    self.hParent  = self:GetParent()
    self.hAbility = self:GetAbility()

    if IsServer() then
        self.nReturnTime   = 0.5
        self.nTimeForCasts = 3
        self.nMaxCasts     = 3

        self.nMaxStamina          = 12
        self.nStaminaRecoverTime  = 3
        self.nStaminaRecoverCount = 2

        self._tLEARNED_ABILITIES_LEVEL = self._tLEARNED_ABILITIES_LEVEL or {}

        self._tFURIOSO_ULTIMATE_UNLOCK = self._tFURIOSO_ULTIMATE_UNLOCK or {}

        self:SetStackCount(self.nMaxStamina)
    end
end
function modifier_roland_light_controller:OnRefresh(tTable)
    self:OnCreated(tTable)
end
function modifier_roland_light_controller:OnIntervalThink()
    local nNewStacks = math.min(self:GetStackCount() + self.nStaminaRecoverCount, self.nMaxStamina)

    self:SetStackCount(nNewStacks)

    if nNewStacks >= self.nMaxStamina then
...
function modifier_roland_light_controller:R_OnUpgrade(hAbility)
    local sCurrentIndex = tostring(hAbility:GetAbilityIndex())

    self._tLEARNED_ABILITIES_LEVEL[sCurrentIndex] = hAbility:GetLevel()
end
function modifier_roland_light_controller:R_RandomNextAbility(Old_hAbility)
    local tRandomAbilitiesUpdatebleCooldown = {}
    local tRandomAbilitiesUpdateblePriority = {}

    local nTotalCountRandom = 0
    for nIndex = 0, self.hParent:GetAbilityCount() - 1 do
        local hAbility = self.hParent:GetAbilityByIndex(nIndex)
        if IsNotNull(hAbility) and type(hAbility.R_CanBeRandomed) == "function" and hAbility.R_CanBeRandomed() then
            nTotalCountRandom = nTotalCountRandom + 1
            if hAbility:IsHidden() then
                if hAbility:IsCooldownReady() then
                    table.insert(tRandomAbilitiesUpdateblePriority, hAbility)
                else
                    table.insert(tRandomAbilitiesUpdatebleCooldown, hAbility)
                end
            end
        end
    end
    ...
    --========-- FURIOSO CHECK
    local hAbilityUltimate = self.hParent:FindAbilityByName("roland_ultimate")
    if IsNotNull(hAbilityUltimate) and hAbilityUltimate:IsHidden() then
        self._tFURIOSO_ULTIMATE_UNLOCK[Old_hAbility:GetAbilityName()] = true
        if TableLength(self._tFURIOSO_ULTIMATE_UNLOCK) >= nTotalCountRandom and hAbilityUltimate:GetHeroLevelRequiredToUpgrade() >= GameRules:GetGameModeEntity():GetCustomHeroMaxLevel() then --AKA 30 MAX
            self.hParent:AddNewModifier(self.hCaster, self.hAbility, "modifier_roland_ultimate", {})
        end
    end
    --========--

    local hNewAbility = TablePick(tRandomAbilitiesUpdatebleCooldown, tRandomAbilitiesUpdateblePriority, true)
    if IsNotNull(hNewAbility) then
        local sCurrentIndex = tostring(Old_hAbility:GetAbilityIndex())

        local nNewLevel = self._tLEARNED_ABILITIES_LEVEL[sCurrentIndex]

        hNewAbility:SetLevel(nNewLevel)

        self.hParent:SwapAbilities(Old_hAbility:GetAbilityName(), hNewAbility:GetAbilityName(), false, true)
    end
end
---------------------------------------------------------------------------------------------------------------------
LinkLuaModifier("modifier_roland_light_max_casts_controll", "heroes/asb_roland/asb_roland", LUA_MODIFIER_MOTION_NONE)

modifier_roland_light_max_casts_controll = modifier_roland_light_max_casts_controll or class({})

function modifier_roland_light_max_casts_controll:IsHidden()                                                                    return false end
function modifier_roland_light_max_casts_controll:IsDebuff()                                                                    return false end
function modifier_roland_light_max_casts_controll:IsPurgable()                                                                  return false end
function modifier_roland_light_max_casts_controll:IsPurgeException()                                                            return false end
function modifier_roland_light_max_casts_controll:RemoveOnDeath()                                                               return true end
function modifier_roland_light_max_casts_controll:DeclareFunctions()
    local tFunc =   {
                    }
    return tFunc
end
function modifier_roland_light_max_casts_controll:OnCreated(tTable)
    self.hCaster  = self:GetCaster()
    self.hParent  = self:GetParent()
    self.hAbility = self:GetAbility()

    if IsServer() then
        self.nReturnTime = tTable.nReturnTime or 0.1

        local vPoint = R_StringSplit(tTable.vStartPoint or self.hParent:GetAbsOrigin())

        self.vStartPoint = Vector(tonumber(vPoint[1]), tonumber(vPoint[2]), tonumber(vPoint[3])) + self.hParent:GetForwardVector()

        local nMaxCasts = tTable.nMaxCasts or 1
        self:SetStackCount(nMaxCasts)
        self:UseCast()
    end
end
function modifier_roland_light_max_casts_controll:OnRefresh(tTable)
    self:OnCreated(tTable)
end
function modifier_roland_light_max_casts_controll:OnDestroy()
    if IsServer() then
        self.hParent:AddNewModifier(self.hCaster, self.hAbility, "modifier_roland_dash_motion", {
                                                                                                    duration      = self.nReturnTime,
                                                                                                    nDashDistance = GetDistance(self.vStartPoint, self.hParent),
                                                                                                    nRecastTime   = 0,
                                                                                                    nIsReturn     = true,

                                                                                                    vPoint = self.vStartPoint
                                                                                                })
    end
end
function modifier_roland_light_max_casts_controll:UseCast()
...
---------------------------------------------------------------------------------------------------------------------
LinkLuaModifier("modifier_roland_recast_time", "heroes/asb_roland/asb_roland", LUA_MODIFIER_MOTION_NONE)

modifier_roland_recast_time = modifier_roland_recast_time or class({})

function modifier_roland_recast_time:IsHidden()                                                                    return false end
function modifier_roland_recast_time:IsDebuff()                                                                    return false end
function modifier_roland_recast_time:IsPurgable()                                                                  return false end
function modifier_roland_recast_time:IsPurgeException()                                                            return false end
function modifier_roland_recast_time:RemoveOnDeath()                                                               return true end
function modifier_roland_recast_time:DeclareFunctions()
    local tFunc =   {
                    }
    return tFunc
end
function modifier_roland_recast_time:OnCreated(tTable)
    self.hCaster  = self:GetCaster()
    self.hParent  = self:GetParent()
    self.hAbility = self:GetAbility()
    ...
    if IsServer() then
        ...
        self.hAbility:RefundManaCost()
    end
end
function modifier_roland_recast_time:OnRefresh(tTable)
    self:OnCreated(tTable)
end
function modifier_roland_recast_time:OnDestroy()
    if IsServer() then
        self.hAbility:UseResources(true, true, true, true)

        local hModifierLightControll = self.hParent:FindModifierByNameAndCaster("modifier_roland_light_controller", self.hCaster)
        if not IsNotNull(hModifierLightControll) then return end
        if type(hModifierLightControll.R_RandomNextAbility) == "function" then
            hModifierLightControll:R_RandomNextAbility(self.hAbility)
        end
    end
end
...
---------------------------------------------------------------------------------------------------------------------
LinkLuaModifier("modifier_roland_dash_motion", "heroes/asb_roland/asb_roland", LUA_MODIFIER_MOTION_HORIZONTAL)

modifier_roland_dash_motion = modifier_roland_dash_motion or class({})

function modifier_roland_dash_motion:IsHidden()                                                                return true end
function modifier_roland_dash_motion:IsDebuff()                                                                return false end
function modifier_roland_dash_motion:IsPurgable()                                                              return false end
function modifier_roland_dash_motion:IsPurgeException()                                                        return false end
function modifier_roland_dash_motion:RemoveOnDeath()                                                           return true end
function modifier_roland_dash_motion:CheckState()
    local tState =  {
...
function modifier_roland_dash_motion:DeclareFunctions()
    local tFunc =   {
                        MODIFIER_PROPERTY_OVERRIDE_ANIMATION
                    }
    return tFunc
end
function modifier_roland_dash_motion:GetOverrideAnimation(keys)
    return ACT_DOTA_OVERRIDE_ABILITY_1
end
function modifier_roland_dash_motion:OnCreated(tTable)
    self.hCaster  = self:GetCaster()
    self.hParent  = self:GetParent()
    self.hAbility = self:GetAbility()
    ...
    if IsServer() then
        ...
        self.nRecastTime = tTable.nRecastTime or 0

        self.vStartLoc = self.hParent:GetAbsOrigin()

        self.nSpeed = (tTable.nDashDistance or 0) / math.max(0.1, self:GetDuration())

        local vPoint = R_StringSplit(tTable.vPoint or self.hParent:GetAbsOrigin())

        self.vPoint = Vector(tonumber(vPoint[1]), tonumber(vPoint[2]), tonumber(vPoint[3])) + self.hParent:GetForwardVector()
        
        self.vMainDirection = GetDirection(self.vPoint, self.vStartLoc)
        self.vMainDistance  = tTable.nDashDistance or 0

        self.bIsReturn = (tTable.nIsReturn or 0) > 0

        if self.bIsReturn then
            self.hParent:SetForwardVector(-self.vMainDirection, true)
            self.hParent:FaceTowards(self.vStartLoc - self.vMainDirection)
        else
            self.hParent:SetForwardVector(self.vMainDirection, true)
        ...
        --     self.nDashPFX =     ParticleManager:CreateParticle("particles/heroes/anime_hero_saito/saito_flashblade_dash.vpcf", PATTACH_ABSORIGIN_FOLLOW, self.hParent)
...
function modifier_roland_dash_motion:OnRefresh(tTable)
    self:OnCreated(tTable)
end
function modifier_roland_dash_motion:OnHorizontalMotionInterrupted()
    if IsServer() then
        self:Destroy()
    end
end
function modifier_roland_dash_motion:UpdateHorizontalMotion(hUnit, nTime)
    if IsServer() then
        ...
        if IsNotNull(self.hAbility) and type(self.hAbility.R_OnWhileDash) == "function" then
            self.hAbility:R_OnWhileDash(self, vNextMovePoint)
        end
...
function modifier_roland_dash_motion:OnDestroy()
    if IsServer() and IsNotNull(self.hParent) then
        ...
        if self.nRecastTime > 0 then
            self.hParent:AddNewModifier(self.hCaster, self.hAbility, "modifier_roland_recast_time", {duration = self.nRecastTime})
        end

        if IsNotNull(self.hAbility) and type(self.hAbility.R_OnAfterDash) == "function" then
            self.hAbility:R_OnAfterDash(self, self.hParent:GetAbsOrigin())
        end
    end
end
...
function modifier_roland_dash_motion:DoEffect(hUnit, vPosition)
    local sImagePFX = "particles/heroes/anime_hero_saito/saito_flashblade_image.vpcf"
    ...
    for _, hEntity in pairs(tEntities) do
        if IsNotNull(hEntity) then
            if self.nAttr_KnockUPDuration > 0 then
                fApplyKnockbackSpecial(hEntity, self.nAttr_KnockUPHeight * 0.5, self.nAttr_KnockUPDuration, hEntity:GetUpVector(), true)
                --=================================--
                --giveUnitDataDrivenModifier(self.hCaster, hEntity, "stunned", self.nAttr_KnockUPDuration)
                hEntity:AddNewModifier(self.hCaster, self.hAbility, "modifier_stunned", {duration = self.nAttr_KnockUPDuration}, hEntity:IsOpposingTeam(self.nCASTER_TEAM))
            end
            --=================================--
            ...
            --=================================--
            DoDamage(self.hCaster, hEntity, self.nImageDamage, self.nDamageType, DOTA_DAMAGE_FLAG_NONE, self.hAbility, false, TableLength(tEntities))
...
---------------------------------------------------------------------------------------------------------------------
LinkLuaModifier("modifier_roland_dash_debuff", "heroes/asb_roland/asb_roland", LUA_MODIFIER_MOTION_NONE)

modifier_roland_dash_debuff = modifier_roland_dash_debuff or class({})

function modifier_roland_dash_debuff:IsHidden()                                                             return false end
function modifier_roland_dash_debuff:IsDebuff()                                                             return false end
function modifier_roland_dash_debuff:IsPurgable()                                                           return true end
function modifier_roland_dash_debuff:IsPurgeException()                                                     return true end
function modifier_roland_dash_debuff:RemoveOnDeath()                                                        return true end
function modifier_roland_dash_debuff:DeclareFunctions()
    local tFunc =   {
                        MODIFIER_PROPERTY_TURN_RATE_OVERRIDE
                    }
    return tFunc
end
function modifier_roland_dash_debuff:GetModifierTurnRate_Override(keys)
    return self.nTurnRate
end
function modifier_roland_dash_debuff:OnCreated(tTable)
    self.hCaster  = self:GetCaster()
    self.hParent  = self:GetParent()
    self.hAbility = self:GetAbility()
...
function modifier_roland_dash_debuff:OnRefresh(tTable)
    self:OnCreated(tTable)
end
