...
LinkLuaModifier( "modifier_aksel_building", "modifiers_re/modifier_aksel_building", LUA_MODIFIER_MOTION_HORIZONTAL )
LinkLuaModifier( "modifier_mars_spear_of_mars_lua_debuff", "modifiers/modifier_mars_spear_of_mars_lua_debuff", LUA_MODIFIER_MOTION_NONE )
...
LinkLuaModifier( "modifier_ice_bitch", "heroes/mars_spear_of_mars_lua", LUA_MODIFIER_MOTION_NONE )

--------------------------------------------------------------------------------
-- Ability Start
function mars_spear_of_mars_lua:GetIntrinsicModifierName()
    return "modifier_ice_bitch"
end
function mars_spear_of_mars_lua:OnUpgrade()
    local ability = self:GetCaster():FindAbilityByName("esdeath_arena")
...
function mars_spear_of_mars_lua:OnSpellStart()
	...
	-- play effects
	local sound_cast = "Esdeath.Spike"
...
function mars_spear_of_mars_lua:OnProjectileHitHandle( target, location, iProjectileHandle )
	...
	-- get stun and damage
	local stun = self:GetSpecialValueFor("stun_duration")+self:GetCaster():FindTalentValue("special_bonus_esdeath_25")
	local damage = self:GetSpecialValueFor("damage")+self:GetCaster():FindTalentValue("special_bonus_esdeath_20")
	...
	local modifier = target:AddNewModifier(
		self:GetCaster(), -- player source
		self, -- ability source
		"modifier_aksel_building", -- modifier name
		{
...
function mars_spear_of_mars_lua:PlayEffects( projID, duration )
	-- Get Resources
	local particle_cast = "particles/mars_spear_impact1.vpcf"
...
modifier_ice_bitch = class ({})
function modifier_ice_bitch:IsHidden() return true end
function modifier_ice_bitch:IsDebuff() return false end
function modifier_ice_bitch:IsPurgable() return false end
function modifier_ice_bitch:IsPurgeException() return false end
function modifier_ice_bitch:RemoveOnDeath() return false end

function modifier_ice_bitch:OnCreated()
    if IsServer() then
...
function modifier_ice_bitch:OnRefresh()
    if IsServer() then
       
    end
end

function modifier_ice_bitch:OnIntervalThink()
    if IsServer() then
        local milf = self:GetParent():FindAbilityByName("esdeath_arena")
        if milf and not milf:IsNull() then
            if self:GetParent():HasScepter() then
                if milf:IsHidden() then
                    milf:SetHidden(false)
                end
            else
                if not milf:IsHidden() then
                    milf:SetHidden(true)
                end
            end
        end
