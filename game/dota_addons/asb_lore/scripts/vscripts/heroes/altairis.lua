altairis = class({})
LinkLuaModifier( "modifier_altair", "modifiers/modifier_altair", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_altair_thinker", "modifiers/modifier_altair_thinker", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_altair_debuff", "modifiers/modifier_altair_debuff", LUA_MODIFIER_MOTION_NONE )
--------------------------------------------------------------------------------
-- Custom KV
-- AOE Radius
function altairis:GetAOERadius()
	return self:GetSpecialValueFor( "destination_radius" )
end

--------------------------------------------------------------------------------
-- Ability Phase Start
function altairis:OnAbilityPhaseStart()
	-- unit identifier
...
function altairis:OnAbilityPhaseInterrupted()
...
--------------------------------------------------------------------------------
-- Ability Start
function altairis:OnSpellStart()
	-- unit identifier
	local caster = self:GetCaster()
	local point = self:GetCursorPosition()
	local vector = point-caster:GetOrigin()
...
--------------------------------------------------------------------------------
-- Projectile
function altairis:OnProjectileHit_ExtraData( target, location, ExtraData )
	...
	self.thinker = CreateModifierThinker(
		caster, -- player source
		self, -- ability source
		"modifier_altair_thinker", -- modifier name
		{ duration = 4 }, -- kv
		location,
		caster:GetTeamNumber(),
		false
	)
	self.thinker = self.thinker:FindModifierByName("modifier_altair_thinker")
...
function altairis:OnProjectileThink_ExtraData( location, ExtraData )
	local returning = ExtraData.returning==1
	if returning then
...
--------------------------------------------------------------------------------
function altairis:PlayEffects1( point, radius )
	-- Get Resources
	local particle_cast = ""
