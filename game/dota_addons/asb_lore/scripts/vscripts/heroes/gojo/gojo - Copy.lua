...
LinkLuaModifier("modifier_gojo_hollow_nuke", "heroes/gojo/gojo.lua", LUA_MODIFIER_MOTION_NONE)
...
function modifier_gojo_hollow_nuke:OnCreated(hTable)
    if IsServer() then
        ...
        -- No sync with animation because duration is higher
       ...
       self.fTravelDelay     = 10.5
       self.fReachHeightTime = 16.0
       self.fHollowNuke      = 17.0
       self.fHollowNukeComb  = 19.0
       self.fHollowNukeEXP   = self.fHollowNuke + 3.0
       ...
       self.fFirstAnimDur    = 122 / self.fFPS
...
function modifier_gojo_hollow_nuke:StagesThinking()
    ...
    if self:GetStage() == GOJO_STAGE_ASCENDING then
        if self.__fElapsedTime >= self.fReachHeightTime 
            ...
            ParticleManager:SetParticleControl(self.iHollowNukeEffect, 2, Vector(500, 0, 0 ))
            self.bSetAnim3 = true
        end
    ...
    if self:GetStage() == GOJO_STAGE_EXPLODING then
        ...
        self.iHollowNukeEffect =  ParticleManager:CreateParticle("particles/test/custom/hollow_purple_nuke_combine_main2.vpcf", PATTACH_ABSORIGIN_FOLLOW, self.parent)
...
function modifier_gojo_hollow_nuke:BallsThinking(me, dt)
    self.vAbstractBluePos = self.vAbstractBluePos or nil
    
    local vCurPos = self.vAbstractBluePos or me:GetOrigin()
    local vGround = GetGroundPosition(vCurPos, me)
    
    if self.__fElapsedTime < self.fTravelDelay then
        local vNewPos = self.fUnitsPerSecBlue * dt
        local vSetPos = vCurPos.z >= vGround.z + self.fDistanceBlue
                        and vCurPos
                        or vCurPos + Vector(0, 0, vNewPos)
...
function modifier_gojo_hollow_nuke:SetStage(nGojoStage)
    self.nHollowNukeSTAGE = nGojoStage or self:GetStage()
end
...
function modifier_gojo_hollow_nuke:Explode()
    ...
    local nHollowNukeFX  = ParticleManager:CreateParticle("particles/test/custom/hollow_purple_nuke_explosion_lightning.vpcf", PATTACH_WORLDORIGIN, nil)
                           ParticleManager:SetParticleControl(nHollowNukeFX, 3, vCasterPos)
    local fHollowNukeDur   = 4.0
    local fHollowNukeTimer = GameRules:GetGameTime() + fHollowNukeDur
    local fHollowNukeTicks = 20.0
    local fHollowNukeInt   = fHollowNukeDur / fHollowNukeTicks
    
    Timers:CreateTimer(0, function()
        if GameRules:GetGameTime() > fHollowNukeTimer or not IsNotNull(self.parent) then
            ParticleManager:DestroyParticle(nHollowNukeFX, true)
        ...
        for _, hEnemy in pairs(hEnemies) do
            if IsNotNull(hEnemy) then
                local fEnemyDMG = (hEnemy:GetMaxHealth() * 0.8) / fHollowNukeTicks
...
function goju_red_orb:OnSpellStart()
    ...
    hCaster:AddNewModifier(hCaster, self, "modifier_gojo_hollow_nuke", { duration = 25.0 })
