...
--===============================================================--
function LerpNumber(nMin, nMax, nTime)
	return nMin + ( nMax - nMin ) * nTime
end
--====================================================================================================--
...
--====================================================================================================--
function GetDirDiff(vDir1, vDir2)
	local nDir1 = VectorToAngles(vDir1).y
	local nDir2 = VectorToAngles(vDir2).y
	return AngleDiff(nDir1, nDir2)
end
...
--====================================================================================================--
LinkLuaModifier("modifier_sukuna_motion_generic", "heroes/sukuna/sukuna_init", LUA_MODIFIER_MOTION_BOTH)

modifier_sukuna_motion_generic = class({})

function modifier_sukuna_motion_generic:IsHidden()										return true end
function modifier_sukuna_motion_generic:IsDebuff()										return self._bIsDebuff end
function modifier_sukuna_motion_generic:IsPurgable()									return self._bIsDebuff end
function modifier_sukuna_motion_generic:IsPurgeException()								return self._bIsDebuff end
function modifier_sukuna_motion_generic:RemoveOnDeath()									return false end
function modifier_sukuna_motion_generic:GetAttributes()									return MODIFIER_ATTRIBUTE_MULTIPLE + MODIFIER_ATTRIBUTE_IGNORE_INVULNERABLE end
function modifier_sukuna_motion_generic:GetPriority()									return MODIFIER_PRIORITY_NORMAL end
function modifier_sukuna_motion_generic:CheckState()
	local t = {}
	t[MODIFIER_STATE_NO_UNIT_COLLISION] = true
	return t
end
function modifier_sukuna_motion_generic:DeclareFunctions()
	local t = 
	{
...
function modifier_sukuna_motion_generic:GetModifierDisableTurning(keys)
	return (self._nFacing ~= 0) and 1 or 0
end
function modifier_sukuna_motion_generic:GetModifierIgnoreCastAngle(keys)
	return self._nFacingCast
end
function modifier_sukuna_motion_generic:OnCreated(tInfo)
	...
	self._nHeight = tInfo.height or 0
...
function modifier_sukuna_motion_generic:OnRefresh(tInfo)
	self:OnCreated(tInfo)
end
function modifier_sukuna_motion_generic:OnHorizontalMotionInterrupted()
	self:Destroy(true)
end
function modifier_sukuna_motion_generic:OnVerticalMotionInterrupted()
	self:Destroy(true)
end
function modifier_sukuna_motion_generic:UpdateHorizontalMotion(hUnit, nDT)
...
function modifier_sukuna_motion_generic:UpdateVerticalMotion(hUnit, nDT)
	local vNow = hUnit:GetAbsOrigin()

	local vStart = self.__vPosSpawn
	local vEnd = self.__vPosTarget

	local nHStart = vStart.z
	local nHEnd = vEnd.z

	local nHTrue = self._nHeight
	local nHMin, nHMax = nHStart, nHEnd
	if nHMin > nHMax then nHMin, nHMax = nHMax, nHMin end
	local nDelta = (nHMax - nHMin) / 2
	local nHMaxTrue = nDelta + nHTrue

	local nKF = self.__nDistancePassed / self:GetDistance(false)
		  nKF = math.clamp(nKF, 0, 1)

	local nNewZ2 = self:GetParabolaZ2(nHStart, nHEnd, nHMaxTrue, nKF)

	local vNew = GetGroundPosition(vNow, hUnit)

	local nGH = vNew.z

	vNew.z = math.max(nGH, nNewZ2)

	local bGTouch = vNew.z <= nGH

	local bGroundEnd = (true and self:GetElapsedTime() > nDT and bGTouch)

	local bDestroy = bGroundEnd or (nKF >= 1)

	hUnit:SetAbsOrigin(vNew)

	if not bDestroy then return end
	self:Destroy(true)
end
function modifier_sukuna_motion_generic:GetParabolaZ2(nHStart, nHEnd, nHMax, nTime)
	return LerpNumber(nHStart, nHEnd, nTime) + nHMax * (4 * nTime * (1-nTime))
end
function modifier_sukuna_motion_generic:GetDistance(bRemaining)
...
function modifier_sukuna_motion_generic:GetSpeed()
...
function modifier_sukuna_motion_generic:IsNavPathable(vPosNow, vPosNew, bTrees, bHG, bBlocked)
...
function modifier_sukuna_motion_generic:IsInterrupted()
	return self._bIsInterrupted or false
end
function modifier_sukuna_motion_generic:OnRemoved(bDeath)
	if not IsServer() then return end
end
function modifier_sukuna_motion_generic:OnRemoved(bDeath)
...
--====================================================================================================--
LinkLuaModifier("modifier_sukuna_animation_generic_self_paused", "heroes/sukuna/sukuna_init", LUA_MODIFIER_MOTION_NONE)

modifier_sukuna_animation_generic_self_paused = class({})

function modifier_sukuna_animation_generic_self_paused:IsHidden()															return true end
function modifier_sukuna_animation_generic_self_paused:IsDebuff()															return false end
function modifier_sukuna_animation_generic_self_paused:IsPurgable()															return false end
function modifier_sukuna_animation_generic_self_paused:IsPurgeException()													return false end
function modifier_sukuna_animation_generic_self_paused:RemoveOnDeath()														return false end
function modifier_sukuna_animation_generic_self_paused:GetAttributes()														return MODIFIER_ATTRIBUTE_MULTIPLE + MODIFIER_ATTRIBUTE_IGNORE_INVULNERABLE end
function modifier_sukuna_animation_generic_self_paused:GetPriority()														return MODIFIER_PRIORITY_SUPER_ULTRA end
function modifier_sukuna_animation_generic_self_paused:CheckState()
	local t = {}
	-- t[MODIFIER_STATE_STUNNED] = true
	-- t[MODIFIER_STATE_COMMAND_RESTRICTED] = true
	return t
end
function modifier_sukuna_animation_generic_self_paused:OnCreated(tInfo)
	self._hCaster  = self:GetCaster()
	self._hParent  = self:GetParent()
	self._hAbility = self:GetAbility()
...
function modifier_sukuna_animation_generic_self_paused:OnRemoved(bDeath)
	if not IsServer() then return end

	if IsNull(self._hParent) or self._hParent:HasModifier("modifier_sukuna_animation_generic_self_paused") then return end
	
	self._hParent:SetForceAttackTargetAlly(nil)
end

--====================================================================================================--
LinkLuaModifier("modifier_sukuna_animation_generic_activity", "heroes/sukuna/sukuna_init", LUA_MODIFIER_MOTION_NONE)

modifier_sukuna_animation_generic_activity = class({})

function modifier_sukuna_animation_generic_activity:IsHidden()															return true end
function modifier_sukuna_animation_generic_activity:IsDebuff()															return false end
function modifier_sukuna_animation_generic_activity:IsPurgable()														return false end
function modifier_sukuna_animation_generic_activity:IsPurgeException()													return false end
function modifier_sukuna_animation_generic_activity:RemoveOnDeath()														return false end
function modifier_sukuna_animation_generic_activity:GetAttributes()														return MODIFIER_ATTRIBUTE_MULTIPLE + MODIFIER_ATTRIBUTE_IGNORE_INVULNERABLE end
function modifier_sukuna_animation_generic_activity:GetPriority()														return MODIFIER_PRIORITY_SUPER_ULTRA end
function modifier_sukuna_animation_generic_activity:DeclareFunctions()
	local t =
	{
...
function modifier_sukuna_animation_generic_activity:GetActivityTranslationModifiers(keys)
	return self._sAct
end
function modifier_sukuna_animation_generic_activity:OnCreated(tInfo)
	self._hCaster  = self:GetCaster()
	self._hParent  = self:GetParent()
	self._hAbility = self:GetAbility()
...
function modifier_sukuna_animation_generic_activity:OnRefresh(tInfo)
	self:OnCreated(tInfo)
end
--====================================================================================================--
LinkLuaModifier("modifier_sukuna_animation_generic", "heroes/sukuna/sukuna_init", LUA_MODIFIER_MOTION_NONE)

modifier_sukuna_animation_generic = class({})

function modifier_sukuna_animation_generic:IsHidden()																return true end
function modifier_sukuna_animation_generic:IsDebuff()																return false end
function modifier_sukuna_animation_generic:IsPurgable()																return false end
function modifier_sukuna_animation_generic:IsPurgeException()														return false end
function modifier_sukuna_animation_generic:RemoveOnDeath()															return false end
function modifier_sukuna_animation_generic:GetAttributes()															return MODIFIER_ATTRIBUTE_MULTIPLE + MODIFIER_ATTRIBUTE_IGNORE_INVULNERABLE end
function modifier_sukuna_animation_generic:GetPriority()															return MODIFIER_PRIORITY_SUPER_ULTRA end
function modifier_sukuna_animation_generic:DeclareFunctions()
	local t = 
	{
		MODIFIER_PROPERTY_DISABLE_TURNING,
		MODIFIER_PROPERTY_IGNORE_CAST_ANGLE,
...
function modifier_sukuna_animation_generic:GetModifierDisableTurning(keys)
	return (self._nFacing ~= 0) and 1 or 0
end
function modifier_sukuna_animation_generic:GetModifierIgnoreCastAngle(keys)
	return self._nFacingCast
end
-- function modifier_sukuna_animation_generic:GetOverrideAnimation(keys)
-- 	return self:GetStackCount()
-- end
-- function modifier_sukuna_animation_generic:GetOverrideAnimationRate(keys)
-- 	return 1
-- end
function modifier_sukuna_animation_generic:OnCreated(tInfo)
	self._hCaster  = self:GetCaster()
	self._hParent  = self:GetParent()
	self._hAbility = self:GetAbility()
	...
	if self._nPause ~= 0 then
		self._hParent:Stop()
		self._hPause = self._hParent:AddNewModifier(self._hParent, self._hAbility, "modifier_sukuna_animation_generic_self_paused", {duration = self._nPause})
	end
	--=================================--
...
function modifier_sukuna_animation_generic:OnRefresh(tInfo)
	self:OnCreated(tInfo)
end
function modifier_sukuna_animation_generic:OnRemoved(bDeath)
	if type(self._bIsInterrupted) == "boolean" then return end
	self._bIsInterrupted = bDeath or (self:GetDuration() > 0 and self:GetRemainingTime() > 0)
end
function modifier_sukuna_animation_generic:OnDestroy()
	if not IsServer() or IsNull(self._hParent) then return end
	--=================================--
...
function modifier_sukuna_animation_generic:Destroy(bInterrupted)
	self._bIsInterrupted = bInterrupted
	return self.BaseClass.Destroy(self)
end
--========================================--
function modifier_sukuna_animation_generic:AddActivities(sActs)
	local _tActs1 = json.decode(sActs) or {}
	local _tActs2 = {}
	for _, sActivity in ipairs(_tActs1) do
		table.insert(_tActs2, self._hParent:AddNewModifier(self._hCaster, self._hAbility, "modifier_sukuna_animation_generic_activity", {duration = 0.1, activity = sActivity}))
	end
	return _tActs2
end
function modifier_sukuna_animation_generic:RemoveActivities(tActs)
...
--========================================--
function modifier_sukuna_animation_generic:IsSpecialAnimation()
	return true
end
function modifier_sukuna_animation_generic:IsInterrupted()
	return self._bIsInterrupted or false
end
function modifier_sukuna_animation_generic:AddCallbackThink(nTime, fCallback)
	if type(fCallback) ~= "function" then return end
...
function modifier_sukuna_animation_generic:AddCallbackEnd(fCallback)
	if type(fCallback) ~= "function" then return end
	table.insert(self._tCB_End, fCallback)
end
--========================================--