function invoker_chaos_meteor_datadriven_on_spell_start(keys)
	...
	local main_damage = ability:GetLevelSpecialValueFor( "main_damage", ability:GetLevel() - 1 )
	local burn_dps = ability:GetLevelSpecialValueFor( "burn_dps", ability:GetLevel() - 1 )
	...
	keys.caster:EmitSound("Barebones.Arena")
	...
	local chaos_meteor_fly_particle_effect = ParticleManager:CreateParticle("particles/ice_meteor_fly.vpcf", PATTACH_ABSORIGIN, keys.caster)
