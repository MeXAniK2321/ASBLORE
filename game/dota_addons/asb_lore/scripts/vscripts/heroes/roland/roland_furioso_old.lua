...
--2:  Allas part-2 long dash (1200) behind/through target
--3:  Boys - knock enemy backward
--4:  Mook - reposition dash if target out of range, then full multi-slash
--5:  Ranga - all 3 slashes (fwd / back / fwd)
--6:  Zelkova - 2 hits with root
--7:  Wheel - AoE strike
--8:  Crystal - 2nd slash only
--9:  Logic big shot 3 backward (large knock range)
--10: Dash to target + Durandal hits 1 & 2 + bonus 3rd hit (hit-1 anim) then snap backward
--====================================================================================================--
...
function roland_furioso:OnCardDashImpact(hCaster, hTarget, vDir)
	...
	--======================================================================--
	-- RANGA (3 parts built in loop)
	--======================================================================--
	tInfo.ranga =
	{
		radius = self:GetSpecialValueFor("ranga_radius")
	}
	local _rDaActs =
	{
	...
	for i = 1, 3 do
		local p =
		{
			...
			bleeds = self:GetSpecialValueFor("ranga_bleeds_"..i),
	...
	tInfo.crystal =
	{
		...
		da_bleeds = self:GetSpecialValueFor("crystal_da_bleeds_2"),
	}
	...
	for _, value in pairs(tInfo) do
		if type(value) == "table" then
			for _key, _value in pairs(value) do
				if string.match(_key, "_time") then
...
function roland_furioso:FuriosoDashTo(tInfo, fOnEnd)
	...
	local function OnEnd(hTarget, bInterrupted)
		if not fOnEnd then return end
		fOnEnd(hTarget, bInterrupted)
	end
...
function roland_furioso:FuriosoLogicCast(tInfo)
	...
	local tAnim =
	{
		...
		activities = json.encode({"logic_cast"}),
		rate = l.ca_rate,
	}
	...
	hAnim:AddCallbackEnd(function(hMod)
		...
		-- Snapshot forward direction for both small shots at cast time
...
--====================================================================================================--
-- STEP 1B: Small shots 1 and 2 (sequential)
--====================================================================================================--
function roland_furioso:FuriosoLogicSmallShot(tInfo, nIndex)
	if nIndex > 2 then
		-- Both small shots done -> proceed to Allas
	...
	local tAnim =
	{
		...
		activities = json.encode({"logic_shot_1"}),
		rate	   = ls.ca_rate,
	}
...
-- tKnock: table with knock_stun_time, knock_time, knock_range, damage
--====================================================================================================--
...
function roland_furioso:OnProjectileHit_ExtraData(hTarget, vLocation, tExData)
	...
	self:AddStun(
	{
		target	= hTarget,
		caster	= hCaster,
		stun_time = tExData.knock_stun_time,
	})
	...
	self:DoDamage(
	{
		target = hTarget,
		caster = hCaster,
		damage = tExData.knock_damage,
	})

	local nPFX = ParticleManager:CreateParticle("particles/heroes/roland/roland_logic/roland_logic_shot_explosion.vpcf", PATTACH_WORLDORIGIN, nil)
				ParticleManager:SetParticleShouldCheckFoW(nPFX, false)
				ParticleManager:SetParticleControl(nPFX, 0, hTarget:GetAbsOrigin() + Vector(0, 0, 50))
				ParticleManager:SetParticleControl(nPFX, 3, hTarget:GetAbsOrigin() + Vector(0, 0, 50))
				ParticleManager:ReleaseParticleIndex(nPFX)

	return false
end
--====================================================================================================--
-- STEP 2A: Allas prep cast (faces target, then dashes through)
--====================================================================================================--
function roland_furioso:FuriosoAllasCast(tInfo)
	...
	local tAnim =
	{
		...
		activities = json.encode({"allas_cast_2"}),
		rate = a.ca_rate,
	}
...
--====================================================================================================--
-- STEP 2B: Allas long dash - passes through target, lands on far side
--====================================================================================================--
function roland_furioso:FuriosoAllasDash(tInfo)
	...
	local tAnim =
	{
		...
		activities = json.encode({"allas_slash_loop"}),
		rate	   = a.da_rate,
	}
	...
	local function OnEnd(hTarget, bInterrupted)
		...
		self:FuriosoDashTo(tInfo, function(hTarget, bInterrupted)
...
--====================================================================================================--
-- STEP 3: Boys - face target and knock backward
--====================================================================================================--
function roland_furioso:FuriosoBoys(tInfo)
	...
	local tAnim =
	{
		...
		pause_sync = 1,
		activity   = ACT_DOTA_CAST_ABILITY_1,
		activities = json.encode({"boys_cast"}),
		rate	   = b.ca_rate,
	}
	...
	self:SetInterruptAndFacing(hAnim, tInfo.target, b.ca_impact_time, 3000, b.ca_stunnable > 0)
...
function roland_furioso:FuriosoMookCast(tInfo)
	...
	local tAnim =
	{
		...
		pause_sync = 1,
		activity   = ACT_DOTA_CAST_ABILITY_1,
		activities = json.encode({"mook_cast_123"}),
		rate	   = m.ca_rate,
	}
...
function roland_furioso:FuriosoMookSlashes(tInfo)
	...
	-- Weapon sheath flash
	local nFlash1 = ParticleManager:CreateParticle("particles/heroes/roland/roland_mook/roland_mook_sheath_flash.vpcf", PATTACH_ABSORIGIN_FOLLOW, tInfo.caster)
					ParticleManager:SetParticleControlEnt(nFlash1, 0, tInfo.caster, PATTACH_POINT_FOLLOW, "attach_attack1", Vector(0,0,0), true)
					ParticleManager:SetParticleControl(nFlash1, 1, Vector(m.radius, 0, 0))
					ParticleManager:ReleaseParticleIndex(nFlash1)

	-- Big slash line from weapon to target
	local nAttachIdx = tInfo.caster:ScriptLookupAttachment("attach_attack1")
	local vAttach = tInfo.caster:GetAttachmentOrigin(nAttachIdx) + Vector(0, 0, -20)
	local vBigDir = GetDirection(vPFXPos, vAttach, true)
	local nBigFX = ParticleManager:CreateParticle("particles/heroes/roland/roland_mook/roland_mook_slash_big.vpcf", PATTACH_WORLDORIGIN, nil)
					...
					ParticleManager:SetParticleControl(nBigFX, 0, vAttach + vBigDir * -m.radius)
					ParticleManager:SetParticleControl(nBigFX, 2, vAttach + vBigDir * m.radius * 2)
					ParticleManager:ReleaseParticleIndex(nBigFX)

	-- Sustained slash circle
	local nSlashesPFX = ParticleManager:CreateParticle("particles/heroes/roland/roland_mook/roland_mook_slashes.vpcf", PATTACH_WORLDORIGIN, nil)
						...
						ParticleManager:SetParticleControl(nSlashesPFX, 1, Vector(m.radius, m.radius, m.radius))
	...
	local tAnim =
	{
		...
		pause_sync = 1,
		activity   = ACT_DOTA_OVERRIDE_ABILITY_1,
		activities = json.encode({"mook_slash_123"}),
	...
	hAnim:AddCallbackThink(0, function(hMod, _tTimer, nTime)
		...
		for _, hEnt in ipairs(tEntities) do
			if IsNotNull(hEnt) and hEnt ~= tInfo.caster then
				...
				local nHitFX = ParticleManager:CreateParticle("particles/heroes/roland/roland_mook/roland_mook_slash_small.vpcf", PATTACH_ABSORIGIN_FOLLOW, hEnt)
								ParticleManager:SetParticleControl(nHitFX, 0, vHit + Vector(0, 0,  100))
								ParticleManager:SetParticleControl(nHitFX, 2, vHit + Vector(0, 0, -100))
	...
	hAnim:AddCallbackEnd(function(hMod)
		...
		local nFlash2 = ParticleManager:CreateParticle("particles/heroes/roland/roland_mook/roland_mook_sheath_flash.vpcf", PATTACH_ABSORIGIN_FOLLOW, tInfo.caster)
						ParticleManager:SetParticleControlEnt(nFlash2, 0, tInfo.caster, PATTACH_POINT_FOLLOW, "attach_attack1", Vector(0,0,0), true)
						ParticleManager:SetParticleControl(nFlash2, 1, Vector(m.radius, 0, 0))
						ParticleManager:ReleaseParticleIndex(nFlash2)
...
function roland_furioso:FuriosoRangaSlash(tInfo, nIndex)
	...
	local function OnHit(hTarget, _nIndex)
		...
		local nFX = ParticleManager:CreateParticle("particles/heroes/roland/roland_mook/roland_mook_slash_small.vpcf", PATTACH_ABSORIGIN_FOLLOW, hTarget)
					ParticleManager:SetParticleControl(nFX, 0, vP1)
					ParticleManager:SetParticleControl(nFX, 2, vP2)
...
--====================================================================================================--
-- STEP 6: Zelkova 2-hit combo with root
--====================================================================================================--
function roland_furioso:FuriosoZelkova(tInfo)
	local z = tInfo.zelkova

	z.sa_act = ACT_DOTA_OVERRIDE_ABILITY_1
	z.sa_act_mod = "zelkova_slash_12"

	if RollPercentage(51) then
...
function roland_furioso:FuriosoWheel(tInfo)
	...
	local tAnim =
	{
		...
		pause_sync = 1,
		activity = ACT_DOTA_CAST_ABILITY_1,
		activities = json.encode({"wheel_cast"}),
		rate = w.ca_rate,
	}
...
function roland_furioso:FuriosoWheelSlash(tInfo)
	...
	local tAnim =
	{
		...
		pause_sync = 1,
		activity = ACT_DOTA_OVERRIDE_ABILITY_1,
		activities = json.encode({"wheel_slash"}),
		rate = w.sa_rate,
	}
	...
	hAnim:AddCallbackThink(w.sa_impact_time, function(hMod)
		if hMod:IsInterrupted() then return end
		local vStrikePos = tInfo.caster:GetAbsOrigin() + tInfo.caster:GetForwardVector() * (w.range or 300)
		local tEntities = FindUnitsInRadius(
			tInfo.team_id,
			vStrikePos,
			nil,
...
function roland_furioso:FuriosoCrystalCast(tInfo)
	...
	local tAnim =
	{
		...
		pause_sync = 1,
		activity = ACT_DOTA_CAST_ABILITY_2,
		activities = json.encode({"crystal_cast_2"}),
		rate = c.ca_rate,
	}
...
function roland_furioso:FuriosoCrystalSlash(tInfo, vDir)
	...
	local tAnim =
	{
		...
		pause_sync = 1,
		activity   = ACT_DOTA_OVERRIDE_ABILITY_2,
		activities = json.encode({"crystal_slash_2"}),
		rate	   = c.da_rate,
	}
	...
	local function OnHit(hTarget, _nIndex)
		...
		-- Cross slash PFX (crystal part-2 signature)
		local vBase = hTarget:GetAbsOrigin() + Vector(0, 0, 128)
		local function MakeCross(vCrossDir, nRad)
			local vS = vBase - vCrossDir * nRad * 0.6
			local vE = vBase + vCrossDir * nRad * 0.6
		...
		MakeCross(RotateVector2D(vDir, 20, true), c.da_range)
		MakeCross(RotateVector2D(vDir, -20, true), c.da_range)
...
--====================================================================================================--
-- STEP 9: Logic big shot 3 (re-face target, fire large-range projectile)
--====================================================================================================--
function roland_furioso:FuriosoLogicBigShot(tInfo)
	...
	local tAnim =
	{
		...
		pause_sync = 1,
		activity = ACT_DOTA_OVERRIDE_ABILITY_2,
		activities = json.encode({"logic_shot_2"}),
		rate = lb.ca_rate,
	}
...
--====================================================================================================--
-- STEP 10B: Durandal hits 1 and 2 (with embedded micro-dash between them)
--====================================================================================================--
function roland_furioso:FuriosoDurandalCast(tInfo)
	...
	local tAnim =
	{
		...
		pause_sync = 1,
		activity = ACT_DOTA_CAST_ABILITY_1,
		activities = json.encode({"durandal_slash_12"}),
		rate = d.ca_rate,
	}
	...
	-- Micro-dash (identical to original Durandal)
	hAnim:AddCallbackThink(d.da_start_time, function(hMod)
		...
		tInfo.caster:AddNewModifier(tInfo.caster, self, "modifier_roland_motion_generic", tMotion)
...
--====================================================================================================--
-- STEP 10C: Bonus 3rd hit (hit-1 animation) + snap backward - THE FINAL MOVE
--====================================================================================================--
function roland_furioso:FuriosoDurandalBonus3rd(tInfo)
	...
	local tAnim =
	{
		...
		activities = json.encode({"durandal_slash_3"}),
		rate = d.ca_rate_3,
	}
	...
	hAnim:AddCallbackEnd(function(hMod)
		if hMod:IsInterrupted() then return end
		if IsNotNull(tInfo.special_invul_target) then
			tInfo.special_invul_target:Destroy()
		end
		if IsNotNull(tInfo.special_invul_caster) then
			tInfo.special_invul_caster:Destroy()
		end
	end)
end
...
--====================================================================================================--
-- SHARED: Apply Durandal slow (modifier_roland_durandal_slow reads "sa_slow_ms_value" from ability)
--====================================================================================================--
