aksel_building = class({})
LinkLuaModifier( "modifier_aksel_building", "modifiers_re/modifier_aksel_building", LUA_MODIFIER_MOTION_HORIZONTAL )
LinkLuaModifier( "modifier_aksel_building_debuff", "modifiers_re/modifier_aksel_building_debuff", LUA_MODIFIER_MOTION_NONE )
...
--------------------------------------------------------------------------------
-- Ability Start
function aksel_building:OnSpellStart()
	-- unit identifier
	local caster = self:GetCaster()
	local point = self:GetCursorPosition() + (caster:GetForwardVector() * 1)

	-- load data
	local projectile_name = "particles/accel_building.vpcf"
	...
	-- play effects
	local sound_cast = "Accel.Building1"
	EmitSoundOn( sound_cast, caster )
	local sound_cast = "Accel.building2"
	EmitSoundOn( sound_cast, caster )
end
...
aksel_building.projectiles = mars_projectiles

-- projectile hit
function aksel_building:OnProjectileHitHandle( target, location, iProjectileHandle )
...
-- projectile think
function aksel_building:OnProjectileThinkHandle( iProjectileHandle )
...
function aksel_building:Pinned( iProjectileHandle )
	local data = self.projectiles[iProjectileHandle]
	...
	data.unit:AddNewModifier(
		self:GetCaster(), -- player source
		self, -- ability source
		"modifier_aksel_building_debuff", -- modifier name
		{
			duration = duration,
			projectile = iProjectileHandle,
		} -- kv
...
--------------------------------------------------------------------------------
function aksel_building:PlayEffects( projID, duration )
	-- Get Resources
