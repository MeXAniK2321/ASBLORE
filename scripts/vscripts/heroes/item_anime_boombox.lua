...
function modifier_item_anime_boombox:OnCreated(table)
    ...
    if RollPercentage(self.theme_chance) then
    	self.SoundForEmit = SoundForEmit.."anime_themes_anime_theme_"..RandomInt(1,2)
   	end

   	if RollPercentage(self.memes_chance) then
    	self.SoundForEmit = SoundForEmit.."anime_memes_anime_meme_"..RandomInt(1,2)
    end
    ...
    if not self.SoundFX then
	    self.SoundFX = ParticleManager:CreateParticle("", PATTACH_OVERHEAD_FOLLOW, self.parent)
...
function modifier_item_anime_boombox:StopAllMusic()
	if IsServer() then
		...
		EmitGlobalSound("item_anime_boombox_cast_"..RandomInt(1, 2))
	end
end