rimuru_water_blade = class({})



--------------------------------------------------------------------------------
-- Ability Start
function rimuru_water_blade:OnUpgrade()
    local ability = self:GetCaster():FindAbilityByName("rimuru_shield")
    if ability and ability:GetLevel() < self:GetLevel() then
        ability:SetLevel(self:GetLevel())
    end
end
function rimuru_water_blade:OnSpellStart()
	-- unit identifier
	...
	-- load data
	local projectile_name = "particles/rimuru_water_slicer.vpcf"
	...
	-- Play effects
	local sound_cast = "rimuru.water_slicer"
...
--------------------------------------------------------------------------------
-- Projectile
function rimuru_water_blade:OnProjectileHitHandle( target, location, projectile )
	...
	local damageTable = {
		...
		damage = self:GetAbilityDamage() + self:GetCaster():FindTalentValue("special_bonus_rimuru_10"),
...
--------------------------------------------------------------------------------
function rimuru_water_blade:PlayEffects( target, direction )
	-- Get Resources
