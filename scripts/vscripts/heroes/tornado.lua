function invoker_tornado_datadriven_on_spell_start(keys)
	...
	local tornado_travel_distance = 3500
	local tornado_lift_duration = 1
	
	
	--Tornado's landing damage bonus is dependent on the level of Wex.
    local ability = keys.ability
	local tornado_landing_damage_bonus = ability:GetLevelSpecialValueFor( "base_damage", ability:GetLevel() - 1 )
	...
	tornado_dummy_unit:EmitSound("Black.tornado")  --Emit a sound that will follow the tornado.
	...
	{
		EffectName = "particles/aksel_tornado.vpcf",
...
function modifier_invoker_tornado_datadriven_cyclone_on_destroy(keys)
	...
	if keys.caster.invoker_tornado_landing_damage_bonus ~= nil then
		...
		ApplyDamage({victim = keys.target, attacker = keys.caster, damage = keys.BaseDamage, damage_type = DAMAGE_TYPE_PURE,})
	end
