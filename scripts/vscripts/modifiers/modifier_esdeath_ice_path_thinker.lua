modifier_esdeath_ice_path_thinker = class({})

--------------------------------------------------------------------------------
-- Classifications
function modifier_esdeath_ice_path_thinker:IsHidden()
	return false
end

function modifier_esdeath_ice_path_thinker:IsDebuff()
	return false
end

function modifier_esdeath_ice_path_thinker:IsStunDebuff()
	return false
end

function modifier_esdeath_ice_path_thinker:IsPurgable()
	return false
end

--------------------------------------------------------------------------------
-- Initializations
function modifier_esdeath_ice_path_thinker:OnCreated( kv )
	self.parent = self:GetParent()
	self.caster = self:GetCaster()

	-- references
	local damage = self:GetAbility():GetSpecialValueFor( "damage" ) + self:GetCaster():FindTalentValue("special_bonus_esdeath_25")
...
function modifier_esdeath_ice_path_thinker:OnRefresh( kv )
end

function modifier_esdeath_ice_path_thinker:OnRemoved()
end

function modifier_esdeath_ice_path_thinker:OnDestroy()
	if not IsServer() then return end
...
--------------------------------------------------------------------------------
-- Interval Effects
function modifier_esdeath_ice_path_thinker:OnIntervalThink()
...
--------------------------------------------------------------------------------
-- Graphics & Animations
function modifier_esdeath_ice_path_thinker:PlayEffects1()
	-- Get Resources
	local particle_cast = "particles/ice_path1.vpcf"
	local sound_cast = "Esdeath.Freeze"
...
function modifier_esdeath_ice_path_thinker:PlayEffects2()
	-- Get Resources
	local particle_cast = "particles/ice_path_b1.vpcf"
